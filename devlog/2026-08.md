...
## 2026-08-31 -- v0.14.0 external palette following

External palette following is an explicit opt-in: a configured local file is
read with a bounded content-hash poller, only complete valid palettes apply
through the existing theme seam, and the last known-good palette is retained
across missing or malformed replacement. Providers cover an explicit
OdyTTY/Base16 complete file plus independent Omarchy-compatible `colors.toml`
and pywal-compatible `colors.json` compatibility. Ordinary default launch
performs no palette discovery or reads. Settings reports the live follower
state, and named-profile appearance can select the same provider and explicit
path under the profile precedence contract. Adversarial tests cover exact
projection, startup isolation, content replacement, malformed input, and
last-known-good recovery. Retained formatting, all-target lint, full test,
production-file guard, and dependency-audit gates pass; the audit reports only
the standing allowed `RUSTSEC-2026-0192` `ttf-parser` maintenance warning.
Public contract:
`docs/v0.14.0-external-palette.md`.

## 2026-08-31 -- v0.14.0 named profile settings manager

Settings gains a Profiles section that opens a local Profile Manager for create,
duplicate, rename, edit, validate, import, export, and confirmed delete. The
manager loads `<config-dir>/profiles` only when opened, keeps unknown future
keys through edit/save (including every nested preserved map), rejects
secret-shaped material before any write, exports through the shared Export
egress, and leaves ordinary default launch unchanged. An observable catalog-load
counter pins that `Settings::from_env` never enumerates profiles. Public
contract: `docs/v0.14.0-profiles-foundation.md`.

## 2026-08-31 -- v0.14.0 named profile foundation

Versioned named launch profiles now have an on-disk schema, bounded local
storage under `<config-dir>/profiles/`, a deterministic precedence resolver,
lossless nested unknown-key round-trip, recursive secret-shaped field rejection,
migration helpers for legacy connection-host appearance fields, and
explicit separation of workspace `default_profile` (connection-host alias) from
named-profile catalog selection. Startup-safe catalog loading scans local files
only. Ordinary default launch behavior is unchanged until a future packet wires
explicit profile selection into menus, CLI, and spawn paths.

Focused tests cover schema round-trip/version skew, malformed-file recovery,
precedence ordering, missing-profile fallback, restored profile hints, and
bounded directory scans. Public contract: `docs/v0.14.0-profiles-foundation.md`.

## 2026-08-31 -- Clarify risky-paste triggers and validation

The public feature reference now gives the complete risky-paste trigger matrix:
the warning is enabled by default, requires child bracketed-paste mode to be
off, and classifies original CR/LF or controls other than Tab without guessing
at shell meaning. It also records direct-path exceptions, exact dialog choices,
preview and one-line bounds, cancellation behavior, source routing, privacy,
and the global advanced opt-out.

The focused manual procedure now holds a neutral child reader open after
disabling bracketed paste. This avoids the misleading Fish case where a prompt
redraw immediately re-enables the mode before the paste occurs. README,
documentation navigation, the settings guide, keybindings, runtime/config
references, SPEC, TODO, and the source ownership comment point to the same
canonical section. Runtime behavior is unchanged.

## 2026-08-31 -- Verify the v0.13.0 release closure

Tag `v0.13.0` points to release commit `2b444659`. Exact-commit blocking CI run
`33386345374` passed on Ubuntu, macOS, and Windows before tagging. Release run
`33387227629` then passed all seven artifact producers and smoke tests, the
locked dependency audit, Minisign checksum signing, constrained GitHub
provenance verification, GitHub Release publication, and Scoop, Homebrew, and
AUR publication.

Independent post-publish checks downloaded all 16 assets, verified every
checksum and the Minisign signature, compared all seven alias/pinned pairs
byte-for-byte, and verified the Linux AppImage plus Windows and macOS archives
against the release-workflow attestation. The pinned source archive built from
its locked graph, and both that build and the packaged Linux binary reported
`odytty 0.13.0`. Public Scoop, Homebrew cask/formula, and AUR metadata carried
the published version and matching hashes. Native macOS and Windows on-device
runtime checks were not performed because maintainer hardware was unavailable;
hosted CI and artifact smoke tests are not relabeled as manual acceptance.

README, SPEC, TODO, the full roadmap, the foundation contract, release guide,
documentation index, notification contract, and manual-validation procedure
now describe the released state and its evidence. A prompt-inclusive
soft-wrapped command-range regression joins the output-only case and confirms
the prompt, typed command, and full rejoined output remain selectable without a
false hard break.

## 2026-08-31 -- Preserve soft-wrapped command boundaries

OSC 133 output-start and command-end marks that share one soft-wrapped logical
line now retain both explicit boundaries and the end offset. If the following
prompt shares that line too, its merged mark carries the same proof forward,
so command copy, search, and export remain available without inferring a range
from displayed text. Zero-offset silent commands continue to fail closed.

The appended snapshot-envelope variants preserve these composite marks across
detach and restore without changing existing wire tags. Focused regressions
cover the original 100-column unterminated output line, range derivation with
and without a following prompt, exact wire bytes, and section round trips.

## 2026-08-31 -- Release v0.13.0 -- Safer command-aware work

Version 0.13.0 adds one cross-platform policy for risky non-bracketed paste,
with a bounded escaped preview and explicit paste, one-line, or cancel choices.
Ordinary single-line and child-enabled bracketed paste retain their existing
byte paths.

Verified OSC 133 command ranges now support output-only and prompt-inclusive
select and copy, command-scoped search, explicit failure navigation, and a
bounded plain-text export path. Missing, malformed, evicted, reset, alternate-
screen, or stale boundaries disable the actions instead of guessing.

Bounded OSC notification and progress handling adds pane-owned completion,
activity, silence, bell, process-finish, and command-failure monitors. Trusted
chrome uses OdyTTY-authored text, terminal-authored payloads remain untrusted,
BEL semantics are unchanged, and native delivery is attempted only on demand
without delaying the first local terminal.

The release carries forward the frozen v0.12.0 performance evidence because it
does not change terminal storage, GPU allocation, or presentation timing. Fresh
performance measurements are not claimed for v0.13.0.

## 2026-08-31 -- Add bounded notification, progress, and pane monitors

OSC 9 and OSC 777 notification requests and OSC 9;4 progress now produce
bounded transient state owned by the emitting pane. Trusted application and
desktop chrome use generic OdyTTY-owned wording rather than terminal-authored
payloads. Queue, payload, rate, duplicate, notice, and progress limits prevent
output from creating persistent or unbounded chrome.

The command palette adds one-shot command-finish, activity, 30-second silence,
bell, process-finish, and explicit command-failure monitors with a clear action.
BEL behavior and settings remain independent, no notification steals focus,
and native Linux, macOS, and Windows delivery is attempted only on demand after
policy checks. The global notification setting defaults to bounded in-app
presentation; named profile overrides remain part of the v0.14.0 profile model.

Focused tests cover hostile and oversized sequences, progress state, queue and
rate bounds, one-shot completion/failure/activity behavior, pane/workspace
ownership, focus policy, restoration defaults, and independent adapter command
specifications. The release roadmap and source ownership comments now describe
the implemented rollup instead of retaining pre-implementation wording.

## 2026-08-31 -- Harden command-range boundaries and cross-platform paste coverage

Command-output actions now retain an unterminated final output fragment when
the following prompt shares its logical line, preserve the boundary through
width-changing reflow, and project soft-wrapped output without inserting false
newlines. Range, selection, search, navigation, and export calculations use the
focused pane's dimensions instead of the containing window grid. A selection
that could identify either of two commands sharing one physical row now fails
closed instead of choosing the older command.

Focused regressions cover reflowed unterminated output, split-pane dimensions,
soft-wrap extraction, and ambiguous shared rows. The cross-platform paste-route
test now accounts for the documented absence of PRIMARY selection on macOS and
Windows while retaining the common-policy assertion for every available text
source. Runtime paste behavior is unchanged.

---

## 2026-08-31 -- Clarify the original Odyssey theme name

The original blue-black palette is now listed as `odyssey-classic`, separating
it clearly from the green `odyssey-default` palette that fresh installations
use. Existing `theme = odyssey` configurations remain byte-compatible through
a legacy alias, while pickers, serialization, documentation, and the automatic
dark-appearance mapping use the descriptive canonical name.

No palette values or default-theme behavior changed.

---

## 2026-08-31 -- Make theme editing direct and state-consistent

Theme Builder role rows now expose their displayed color as a direct hex field.
Clicking the value or pressing Enter opens `#rgb` / `#rrggbb` entry; the first
typed digit replaces the prefilled value, while Enter alone keeps it unchanged.
Label clicks still focus a role for slider and keyboard adjustment without
forcing edit mode.

Successful theme-picker saves now apply the canonical reloaded settings before
returning to the parent Settings page. Theme Builder saves also reload even when
the config already names that theme, because the theme file itself may have
changed. This removes the state split where preview colors could change while
the Settings page retained an older theme name.

---

## 2026-08-31 -- Refine Red Planet Dark

Red Planet Dark now pairs its `#161616` canvas with a darker, dustier iron-red
foreground and cursor instead of the published palette's sand foreground. Its
normal and bright reds move away from orange, while normal and bright blues gain
definition against background effects. Red Planet remains byte-faithful to the
published palette; only the OdyTTY Dark companion receives these tuned roles.

The built-in invariant test locks the four adjusted ANSI slots, foreground,
cursor, canvas, and all unchanged semantic roles independently.

---

## 2026-08-30 -- Add semantic command-output actions

Complete, ordered OSC 133 command boundaries now produce opaque ranges bound to
the live terminal generation. The existing grid and scrollback remain the only
text model. Output-only and prompt-inclusive select/copy actions, command-scoped
search, previous/next explicit failure navigation, and plain-text export are
available from the command palette and terminal context menu and through all 48
rebindable action slots. Missing, partial, malformed, evicted, reset,
alternate-screen, or stale boundaries disable the actions instead of guessing.
Context-menu authority is also bound to the session that opened the menu.

Export uses the exactly pinned `rfd` 0.17.2 async native save adapter with its
minimal portal feature. The adapter receives only an OdyTTY-owned neutral
filename and filter; terminal output cannot choose a path. After selection, the
range is revalidated and the canonical visible cell-text projection excludes
escape sequences, OSC working-directory metadata, hyperlink targets, and image
payloads. A 32 MiB whole-export cap, final symlink/reparse refusal,
owner-private exclusive sibling temporary file, destination identity recheck,
sync, and atomic replacement bound filesystem egress without creating or
changing the selected parent directory.

The command-range implementation moved to its designated source module and the
48-action model moved out of the settings model, keeping handwritten production
files below the project limit. Public-safe Bash, Zsh, Fish, and PowerShell
fixtures cover selection, copy, export projection, missing and stale marks,
alternate-screen refusal, scoped search, explicit failures, hyperlinks, private
cwd metadata, and inline images. Writer tests cover the exact cap, cap plus one,
new and replacement files, unchanged parents, missing parents, and Unix final
symlink refusal. Platform-specific native dialog and writer evidence remains a
release gate rather than being inferred from the Linux build.

---

## 2026-08-30 -- Add Red Planet theme pair

The built-in theme library now includes the published Red Planet palette and an
OdyTTY Red Planet Dark companion. `red-planet` preserves the upstream
foreground, background, cursor, selection, and sixteen ANSI colors. The initial
`red-planet-dark` form lowered the background and clear roles from `#222222` to
`#161616`; the later August 31 refinement tunes its foreground, cursor, red, and
blue roles while retaining that deeper canvas.

Both themes use the ordinary built-in `.theme` parser and appear in the theme
picker, `--list-themes`, and documentation roster. The roster count is now 144,
the upstream creator and collection are attributed, and a focused invariant test
locks the darker variant's deliberate differences and unchanged semantic roles.
