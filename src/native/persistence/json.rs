...
//! A tiny, self-contained JSON reader/writer for the workspace shape snapshot.
//!
//! `serde_json` is not in the dependency tree, and the shape schema is fixed
//! and shallow (objects, arrays, strings, numbers, booleans, null), so a small
...
impl Json {
    ...
    pub(super) fn obj<const N: usize>(entries: [(&str, Json); N]) -> Json {
        Json::Obj(
            entries
                .into_iter()
    ...
    pub(super) fn as_owned_str(&self) -> Option<String> {
        match self {
...
pub(super) fn to_pretty(value: &Json) -> String {
