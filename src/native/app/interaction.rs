...
fn click_travel_delta(
    ...
    // NF18/A2 (Windows/ConPTY only): at the append origin — the caret one past
    // the last input glyph — PSReadLine under ConPTY emits a terminal cursor one
    // edit-position RIGHT of its true buffer caret. The `RightEdgeUnknown` click
    // path has no cursor-vs-signal validation (unlike the Exact/OSC-133 path, so
    // fish stays correct), so it trusts that emitted cursor and a left-travel
    // from the append origin over-shoots by exactly one press. Pull the source
    // caret back one glyph, but ONLY at the append origin (cursor at the region's
    // end column on its last row) and ONLY under `RightEdgeUnknown`: a blanket
    // shift would double-correct a subsequent mid-line click once the model
    // re-syncs to `click_col`. Linux/macOS shells report an accurate cursor on
    // this path, so the correction is Windows-scoped; the Exact path is
    // byte-identical on every platform. Pending on-device confirmation.
    #[cfg(windows)]
    let cursor_flat = if region.certainty == crate::core::InputCertainty::RightEdgeUnknown
        && cursor.row == region.end_row
        && cursor.column == region.end_col
    {
...
mod tests {
    ...
    // --- NF18/A2: PowerShell append-position click-to-place correction ---
    //
    // Reproduces the confirmed Windows trace geometry (single ASCII row, 20 input
    // glyphs at cells [34..53], append origin at column 54) and asserts the
    // `click_travel_delta` model directly. A1 (global ConPTY cursor bias) and A3
    // (arrow/redraw race) were ruled out by operator capture; the sole surviving
    // cause is the append-origin cursor over-count on the un-validated
    // `RightEdgeUnknown` path, corrected Windows-only.

    /// Absolute input geometry from the trace: 20 glyphs in `[34, 54)` on row 0.
    const NF18_START_COL: usize = 34;
    const NF18_END_COL: usize = 54; // exclusive append column
    const NF18_LEN: usize = NF18_END_COL - NF18_START_COL; // 20 glyphs

    /// A single-row snapshot with `len` ASCII glyphs starting at `start` on row 0.
    ...
    /// The trace region: single row, `RightEdgeUnknown` (PowerShell/bash — no
    /// OSC 133 input-end signal), heuristic end at the append column.
    ...
    /// L1 of the trace: click at col 46 with the caret at the append origin
    /// (col 54). Uncorrected travel is 8 Left presses (delta -8).
    fn nf18_append_delta(certainty: InputCertainty) -> Option<i32> {
        click_travel_delta(
            &nf18_snapshot(),
            &nf18_region(certainty),
            CellPoint { row: 0, column: 46 },
            Position {
                row: 0,
                column: NF18_END_COL,
            },
    ...
    #[cfg(windows)]
    #[test]
    fn nf18_append_origin_corrects_one_left_on_windows() {
        // L1: caret at the append origin under RightEdgeUnknown. PSReadLine's
        // emitted cursor is one right of its true caret, so the fix pulls the
        // source caret back one glyph: 7 Left presses, not 8.
        assert_eq!(
            nf18_append_delta(InputCertainty::RightEdgeUnknown),
            Some(-7)
        );
    }
    ...
    #[cfg(not(windows))]
    #[test]
    fn nf18_append_origin_uncorrected_off_windows() {
        // The same geometry on Linux/macOS (bash / fish mid-edit report an
        // accurate cursor): the correction is Windows-scoped, so travel stays the
        // literal 8 Left presses — no off-by-one is introduced where none exists.
        assert_eq!(
            nf18_append_delta(InputCertainty::RightEdgeUnknown),
            Some(-8)
        );
    ...
    #[test]
    fn nf18_midline_click_is_never_corrected() {
        // L3: after a click the caret sits mid-line (col 46, not the append
        // column 54), so the append gate does NOT fire on any platform. Click at
        // col 41 → 5 Left presses, identical everywhere. Guards against a blanket
        // RightEdgeUnknown shift double-correcting a re-synced model.
        let delta = click_travel_delta(
            &nf18_snapshot(),
            &nf18_region(InputCertainty::RightEdgeUnknown),
            CellPoint { row: 0, column: 41 },
            Position { row: 0, column: 46 },
            0,
            28,
        );
    ...
    #[test]
    fn nf18_exact_path_append_is_byte_identical() {
        // The fish / OSC 133 Exact path validates its cursor and must stay
        // byte-identical on EVERY platform: even at the append origin the
        // correction never fires (it is gated on RightEdgeUnknown), so the delta
        // is the literal 8 Left presses on Windows too.
        assert_eq!(nf18_append_delta(InputCertainty::Exact), Some(-8));
    }
}
