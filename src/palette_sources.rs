...
//! caller-provided path into memory, never logs entries, never persists them,
//! and never transmits them. Tests use synthetic temp-file fixtures only.
...
/// with consecutive duplicates collapsed after reversing.
...
/// file and never returns more than `limits.max_entries`.
pub fn read_shell_history_with_limits(
    path: impl AsRef<Path>,
    format: ShellHistoryFormat,
    limits: HistoryReadLimits,
) -> Vec<String> {
    if limits.max_bytes == 0 || limits.max_entries == 0 || limits.max_entry_chars == 0 {
        return Vec::new();
    }
    ...
    for line in text.lines().rev() {
        let Some(entry) = parse_history_line(line, format, limits.max_entry_chars) else {
            continue;
        };
...
mod tests {
    ...
    #[test]
    fn bash_history_is_most_recent_first_and_collapses_consecutive_duplicates() {
