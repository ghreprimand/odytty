...
/// Parse the colored-background opacity floor (`ODYTTY_COLORED_BG_OPACITY`).
/// `0.0` disables the floor (colored cells composite exactly like default-
/// background cells); the `0.9` default keeps colored blocks strong under a
/// translucent window. Out-of-range or invalid values warn and fall back to
/// the default; valid values clamp to `[0,1]`.
pub(super) fn parse_colored_bg_opacity(raw: Option<&OsStr>, warn: &mut impl FnMut(&str)) -> f32 {
    let Some(raw) = raw else {
        return DEFAULT_COLORED_BG_OPACITY;
    };
    let value = raw.to_string_lossy();
    let trimmed = value.trim();
    if trimmed.is_empty() {
    ...
    let parsed = match trimmed.parse::<f32>() {
        ...
        _ => {
            warn(&format!(
                "{COLORED_BG_OPACITY_ENV}={trimmed:?} is not a valid opacity; using {DEFAULT_COLORED_BG_OPACITY}"
            ));
            return DEFAULT_COLORED_BG_OPACITY;
        }
    };
...
pub(super) fn parse_cursor_glow_intensity(raw: Option<&OsStr>, warn: &mut impl FnMut(&str)) -> f32 {
    let Some(raw) = raw else {
        return DEFAULT_CURSOR_GLOW_INTENSITY;
    };
    let value = raw.to_string_lossy();
    let trimmed = value.trim();
    if trimmed.is_empty() {
    ...
    let parsed = match trimmed.parse::<f32>() {
        ...
        _ => {
            warn(&format!(
                "{CURSOR_GLOW_INTENSITY_ENV}={trimmed:?} is not a valid cursor glow intensity; using {DEFAULT_CURSOR_GLOW_INTENSITY}"
            ));
            return DEFAULT_CURSOR_GLOW_INTENSITY;
        }
    };
...
pub(super) fn bindable_action_name(action: BindableAction) -> &'static str {
    match action {
        ...
        BindableAction::SelectCommandOutput => "select-command-output",
        BindableAction::SelectCommandWithPrompt => "select-command-with-prompt",
        BindableAction::CopyCommandOutput => "copy-command-output",
        BindableAction::CopyCommandWithPrompt => "copy-command-with-prompt",
        BindableAction::SearchCommandOutput => "search-command-output",
        ...
        BindableAction::PrevWorkspace => "prev-workspace",
        BindableAction::WorkspacePicker => "workspace-picker",
