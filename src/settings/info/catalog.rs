    ...
    pub fn setting_info(&self) -> Vec<SettingInfo> {
        let mut rows = vec![
            ...
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal",
                env: QUICK_TERMINAL_ENV,
                name: "Quick terminal",
                value: bool_display(self.quick_terminal).to_owned(),
                description: "A dedicated, summonable drop-down terminal with its own window, geometry, and preserved session, kept separate from ordinary windows and ordinary window restoration. Off by default (opt-in): when off no global shortcut is registered and no dedicated window is created, so startup is unchanged. Whether a global summon shortcut can be granted is platform-dependent (for example on Wayland the compositor, not the application, owns global grabs) and reported after the first usable frame.",
                kind: SettingKind::Bool,
                range: None,
                options: &["on", "off"],
                reloadable: false,
            ...
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal_shortcut",
                env: QUICK_TERMINAL_SHORTCUT_ENV,
                name: "Quick terminal shortcut",
                value: self.quick_terminal_shortcut.clone(),
                description: "The global shortcut accelerator that summons or hides the quick terminal, e.g. F12 or ctrl+shift+grave. Only meaningful when quick_terminal is on. Recognizes ctrl, alt, shift, and super/cmd modifiers plus one key. Whether the operating system can grant the global grab is platform-dependent and reported after the first usable frame.",
                kind: SettingKind::String,
                range: None,
                options: &[],
                reloadable: false,
            ...
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal_edge",
                env: QUICK_TERMINAL_EDGE_ENV,
                name: "Quick terminal edge",
                value: self.quick_terminal_edge.clone(),
                description: "Which monitor edge the quick terminal anchors to: top (the conventional drop-down placement and default), bottom, left, or right. Only meaningful when quick_terminal is on.",
                kind: SettingKind::String,
                range: None,
            ...
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal_coverage",
                env: QUICK_TERMINAL_COVERAGE_ENV,
                name: "Quick terminal coverage",
                value: self.quick_terminal_coverage.clone(),
                description: "How far the quick terminal extends from its anchored edge, as a percentage of the monitor work area (e.g. 40%) or an absolute pixel size (e.g. 600px). Percentages scale with the display and are the accessible default.",
                kind: SettingKind::String,
                range: None,
                options: &[],
                reloadable: false,
            ...
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal_span",
                env: QUICK_TERMINAL_SPAN_ENV,
                name: "Quick terminal span",
                value: self.quick_terminal_span.clone(),
                description: "How wide (for top/bottom edges) or tall (for left/right edges) the quick terminal is, as a percentage of the monitor work area (e.g. 100%) or an absolute pixel size. Defaults to spanning the full edge.",
                kind: SettingKind::String,
                range: None,
                options: &[],
                reloadable: false,
            ...
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal_monitor",
                env: QUICK_TERMINAL_MONITOR_ENV,
                name: "Quick terminal monitor",
                value: self.quick_terminal_monitor.clone(),
                description: "Which monitor the quick terminal appears on: active (the monitor of the focused window, falling back to any open window then the primary; pointer position is not used), primary (always the primary monitor), or a fixed monitor index. A fixed index that is gone at summon time falls back to the active monitor rather than a silent no-show.",
                kind: SettingKind::String,
                range: None,
                options: &[],
                reloadable: false,
            ...
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal_animation",
                env: QUICK_TERMINAL_ANIMATION_ENV,
                name: "Quick terminal animation",
                value: self.quick_terminal_animation.clone(),
                description: "The reveal animation for the quick terminal: instant (no motion, the default) or slide (slides in from the anchored edge). Always forced to instant when reduced_motion is on, regardless of this setting.",
                kind: SettingKind::String,
                range: None,
                options: &["slide", "instant"],
                reloadable: false,
                numeric: None,
            },
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal_hide_on_focus_loss",
                env: QUICK_TERMINAL_HIDE_ON_FOCUS_LOSS_ENV,
                name: "Quick terminal hide on focus loss",
                value: bool_display(self.quick_terminal_hide_on_focus_loss).to_owned(),
                description: "When off (the default), the quick terminal stays visible until summoned away. When on it hides automatically as soon as it loses focus so it never lingers over other work.",
                kind: SettingKind::Bool,
                range: None,
                options: &["on", "off"],
                reloadable: false,
            ...
            SettingInfo {
                group: "Sessions",
                key: "quick_terminal_profile",
                env: QUICK_TERMINAL_PROFILE_ENV,
                name: "Quick terminal profile",
                value: if self.quick_terminal_profile.trim().is_empty() {
                    "(default profile)".to_owned()
                } else {
                    self.quick_terminal_profile.clone()
                },
                description: "Optional profile name the quick terminal session launches with. Empty (the default) uses the default profile, so the quick terminal needs no profile configuration to work.",
                kind: SettingKind::String,
                range: None,
                options: &[],
                reloadable: false,
