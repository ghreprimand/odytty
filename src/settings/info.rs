...
impl Settings {
    ...
    pub fn setting_info(&self) -> Vec<SettingInfo> {
        let mut rows = vec![
            ...
            SettingInfo {
                ...
                name: "Rail width",
                value: format_float(self.tab_rail_width),
                description: "Width of the vertical tab rail in cells. Only affects the left/right rail placements; the top bar ignores it. Applies live.",
                kind: SettingKind::Number,
                range: None,
                options: &[],
                reloadable: true,
                numeric: Some(NumericSpec {
                    min: MIN_TAB_RAIL_WIDTH,
                    max: MAX_TAB_RAIL_WIDTH,
                    step: 1.0,
                    unit: "cells",
                }),
            },
    ...
    pub fn display_value_for_key(&self, key: &str) -> Option<String> {
        let value = match key {
            ...
            "tab_rail_width" => format_float(self.tab_rail_width),
