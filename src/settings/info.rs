    ...
    pub fn setting_info(&self) -> Vec<SettingInfo> {
        let mut rows = vec![
            ...
            SettingInfo {
                ...
                description: "When on, the cursor eases its opacity in and out across each blink instead of switching hard on and off. Off by default; only acts while the cursor is blinking and the window is focused. Purely visual.",
                kind: SettingKind::Bool,
                range: None,
                options: &["on", "off"],
                reloadable: true,
            ...
            SettingInfo {
                ...
                description: "When on, a short fading after-image trails the cursor as it glides between cells, in the theme cursor color and drawn behind the cursor block. Off by default; only visible while Cursor slide is also on (it trails that motion) and fully decays as the glide settles. Purely visual — never moves the logical cursor.",
                kind: SettingKind::Bool,
                range: None,
                options: &["on", "off"],
                reloadable: true,
            ...
            SettingInfo {
                ...
                description: "What plain Ctrl+C does. Off (default) always sends the interrupt signal (^C), as a terminal normally does. Copy-or-interrupt: when text is selected, Ctrl+C copies it and clears the selection; with nothing selected, Ctrl+C still sends the interrupt. To still interrupt while text is selected, press Esc first (or Ctrl+C twice). Ctrl+Shift+V pastes, and Ctrl+Shift+C always copies, regardless of this setting.",
                kind: SettingKind::Enum,
                range: None,
