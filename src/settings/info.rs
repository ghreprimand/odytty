    ...
    pub fn setting_info(&self) -> Vec<SettingInfo> {
        let mut rows = vec![
            ...
            SettingInfo {
                ...
                description: "Links the nearby cursor echo and the large-jump streak under one intensity profile. balanced preserves the default restrained echo; subtle reduces opacity, radius, lag, and duration; expressive raises them within the same bounded schedule. The logical and rendered cursor still reaches every destination immediately.",
                kind: SettingKind::Enum,
                range: None,
