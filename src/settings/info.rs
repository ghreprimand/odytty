    ...
    pub fn setting_info(&self) -> Vec<SettingInfo> {
        let mut rows = vec![
            ...
            SettingInfo {
                ...
                description: "The sidebar listing workspaces: auto (default, appears once a second workspace exists), always (pinned even with one), or left/right (pinned to that side). Auto and always inherit the side from tab bar placement. Applies live.",
                kind: SettingKind::Enum,
                range: None,
                options: &["auto", "always", "left", "right"],
                reloadable: true,
                numeric: None,
            },
            SettingInfo {
                ...
                description: "Vertical tab rail width: \"auto\" sizes to the longest tab title (up to the rail max width), or a fixed cell count. Drag the rail's inner edge to set a manual width; double-click it to return to auto. Rail placements only. Applies live.",
                kind: SettingKind::String,
                range: Some(format!(
                    "auto or {}-{} cells",
            ...
            SettingInfo {
                ...
                description: "Widest the auto-sized rail grows to fit long tab titles before truncating them with an ellipsis. Only used when the rail width is \"auto\". Rail placements only. Applies live.",
                kind: SettingKind::Number,
                range: None,
                options: &[],
                reloadable: true,
            ...
            SettingInfo {
                ...
                description: "Rows of empty space between adjacent rail slots (the top margin before the first slot follows it). Rail placements only. Applies live.",
                kind: SettingKind::Number,
                range: None,
                options: &[],
                reloadable: true,
            ...
            SettingInfo {
                ...
                description: "Rows each rail slot occupies: 1 for a compact single-row list (labels truncate), 2 for the padded default that can wrap a long title across two rows. Rail placements only. Applies live.",
                kind: SettingKind::Number,
                range: None,
                options: &[],
                reloadable: true,
            ...
            SettingInfo {
                group: "Input",
                key: "smooth_scroll",
                env: SMOOTH_SCROLL_ENV,
                name: "Smooth scrolling",
                value: bool_display(self.smooth_scroll).to_owned(),
                description: "When on, scrollback movement glides into place over a short bounded ease instead of jumping. Off by default. The scroll position updates immediately (no added input latency); only the visual catches up. Programmatic jumps and active drag-autoscroll snap.",
                kind: SettingKind::Bool,
                range: None,
                options: &["on", "off"],
                reloadable: true,
