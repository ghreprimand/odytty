...
- **Shipped — Drag-to-reorder workspaces in the rail.** A press-drag-drop gesture on a rail slot, a feel-polish complement to the menu reorder: press feedback lifts the grabbed slot immediately, a small movement threshold disambiguates a click (switch) from a drag, and a floating proxy follows the live drop position alongside the bright insertion rule. Drop targeting excludes the lifted workspace, while the resting slots part around its reserved destination so labels remain distinct and intact. Release commits and repaints the new order immediately; Escape cancels with the order untouched. Reuses the shipped `move_workspace` engine for the commit, so active-follow-by-identity and shape-autosave persistence are identical to the menu path; the auto-hide rail is held open for the whole gesture. Cross-platform pointer path, no platform-specific surface.
