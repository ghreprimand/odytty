...
| Prompt key enhancement | In integrated Bash and Zsh prompts, gives `Ctrl+Backspace`, `Shift+Enter`, and `Ctrl+Enter` distinct word-edit, newline, and submit behavior. Existing personal bindings win. | `shell_key_enhancement = off` restores plain prompt input |
| Click-to-position and command status gutter | Moves the prompt cursor on a supported click and marks completed commands green or red. Both stay inert without shell marks. | `sh_click = off` and `command_status_gutter = off` |
