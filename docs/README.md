# OdyTTY Documentation

[Project overview](../README.md) | [Website](https://odytty.unfinished-works.com)

This is the complete map of OdyTTY's tracked project documentation.

## Start Here

- [Install guide](install.md) — setup, updates, checksums, source builds, and
  troubleshooting.
- [Feature reference](features.md) — terminal behavior and native workflows.
- [Settings guide](settings-guide.md) — shipped defaults and useful opt-ins.
- [Runtime reference](runtime-knobs.md) — every setting and command-line
  surface.

## Use And Customize

- [Annotated config](odytty.conf.example) — a commented configuration starting
  point.
- [Keybindings](keybindings.md) — shortcuts and rebinding.
- [Buttons](buttons.md) — the program-defined clickable-output protocol.
- [Accessibility](accessibility.md) — readability and motion controls.
- [Diagnostics](diagnostics.md) — logs, recovery, and support information.

## Visual System

- [Themes](themes.md)
- [Effects](effects.md)
- [Graphics](graphics.md)
- [Shaping roadmap](shaping-roadmap.md)
- [Visual architecture](visual-architecture.md)
- [HiDPI validation](hidpi-validation.md)

Together these cover the renderer, visual effects, inline media, themes, and
scaling behavior.

## Architecture And Design

- [Product specification](../SPEC.md)
- [Interactive paths design](interactive-paths-design.md)
- [Panes and sessions design](panes-and-sessions-design.md)
- [Session attach launcher design](session-attach-launcher-design.md)
- [v0.13.0 foundation contract](v0.13.0-foundation.md)

## Stabilization And Evidence

- [Pre-1.0 acceptance contract](pre-1.0-acceptance.md)
- [Stabilization and release-channel policy](stabilization-policy.md)
- [Historical stabilization baseline](stabilization-baseline.md)
- [v0.10.0 baseline](v0.10.0-baseline.md)
- [Coverage evidence](coverage-evidence.md)
- [Mutation-testing policy](mutation-testing.md)
- [Manual-validation matrix](manual-validation.md)
- [Real-application smoke matrix](compatibility/real-application-smoke.md)
- [Compatibility corpus and regression intake](compatibility/corpus.md)
- [Optional external daily-driver protocol](external-daily-driver.md)
- [Parser and graphics fuzzing guide](../fuzz/parser_graphics/README.md)
- [Native decomposition record](native-decomposition.md)
- [Comparative benchmark protocol](benchmark-protocol.md)
- [Benchmark harness and operator guide](../scripts/bench-protocol/README.md)
- [Comparative benchmark apparatus statement](benchmark-apparatus.md)
- [Comparative benchmark results: W6 idle and software endpoints](benchmark-results.md)
- [Memory model, measurement, and target](memory.md)
- [Terminal threat model](threat-model.md)
- [Pinned vttest methodology and results](compatibility/vttest.md)
- [Dynamic-analysis lanes](dynamic-analysis.md)

These documents distinguish reproducible evidence and executed results from
unsupported, skipped, deferred, and unmeasured surfaces.

## Packaging And Release

- [Packaging guide](../PACKAGING.md)
- [Release guide](release.md)
- [AUR runbook](../dist/aur/README.md)
- [Homebrew runbook](../dist/homebrew/README.md)

## History And Roadmap

- [Current work](../TODO.md)
- [Development log index and monthly archives](../DEVLOG.md)
- [Full build roadmap](full-build-roadmap.md)
- [Idle-wakeups investigation](idle-wakeups-investigation.md)

## Contributing And Security

- [Contributing](../CONTRIBUTING.md)
- [Security policy](../SECURITY.md)
- [License](../LICENSE)
- [Branding notice](../NOTICE)
