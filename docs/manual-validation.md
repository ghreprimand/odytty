...
| W-DROP-01 | External file drag and drop | Dropping a file onto the terminal shows the exact notice `File drop insertion is not available on Windows in this version. Copy the path manually.` and writes nothing to the shell; no preview opens. Internal tab, workspace, divider, and selection drags are covered separately and must not be cited as file-drop support. |  |  |
